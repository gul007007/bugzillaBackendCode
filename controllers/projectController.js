...
exports.updateProject = async (req, res) => {
  try {
    ...
    if (project.managerId.toString() !== req.session.user.id) {
    ...
    // Resolve and validate developer emails to user IDs
    if (developerEmails && developerEmails.length > 0) {
      const developers = await User.find({
        email: { $in: developerEmails },
        role: developerRole._id,
      ...
      if (developers.length !== developerEmails.length) {
        return res
          .status(400)
          .json({ error: "One or more developer emails not found or not a Developer" });
      }
      project.developerIds = developers.map((user) => user._id);
    } else {
      project.developerIds = []; // Clear developerIds if no emails provided
    }

    // Resolve and validate QA emails to user IDs
    if (qaEmails && qaEmails.length > 0) {
      ...
      if (qas.length !== qaEmails.length) {
        return res
          .status(400)
          .json({ error: "One or more QA emails not found or not a QA" });
      }
      project.qaIds = qas.map((user) => user._id);
    } else {
      project.qaIds = []; // Clear qaIds if no emails provided
    }
